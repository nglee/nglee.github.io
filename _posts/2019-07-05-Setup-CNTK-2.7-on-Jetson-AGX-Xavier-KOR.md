...
Xavier가 낼 수 있는 최고의 성능으로 작업을 진행하기 위해 다음의 명령을 수행합니다.
```
$ sudo nvpmodel -m 0
```
...
이제 CNTK 설치를 위한 사전작업으로 다음의 라이브러리들을 설치하겠습니다.
...
$ sudo make install TARGET=ARMV8 PREFIX=/usr/local/OpenBLAS
```

## Boost 1.68.0

```
$ git clone -q --depth 1 -b boost-1.68.0 --recursive -j $(nproc) https://github.com/boostorg/boost.git
$ cd boost
...
```
$ sudo mkdir /usr/local/openmpi/bin -p
$ sudo mkdir /usr/local/openmpi/include -p
$ sudo ln -s /usr/bin/mpic++ /usr/local/openmpi/bin/mpic++
$ sudo ln -s /usr/include/mpi/mpi.h /usr/local/openmpi/include/mpi.h

$ sudo ln -sf /usr/local/cuda/include/nvml.h /usr/local/include
$ sudo ln -sf /usr/local/cuda/lib64/stubs/libnvidia-ml.so /usr/local/lib/libnvidia-ml.so.1
...
$../../configure --asgd=no \
                 ...
                 --with-opencv=/usr \
                 --with-boost=/usr/local/boost-1.68.0 \
...
CNTK가 제대로 빌드되었는지 확인하기 위해 [여기](https://docs.microsoft.com/en-us/cognitive-toolkit/Setup-CNTK-on-Linux#quick-test-of-cntk-build-functionality)에 나온 테스트 스텝을 따라할 수 있습니다. 그런데 Jetson 계열이 사용하는 Tegra 플랫폼은 NVML(NVIDIA Management Library) 이용과 관련해서 조심해야 할 부분이 있습니다. [여기](https://devtalk.nvidia.com/default/topic/999740/jetson-tx1/-libnvidia-ml-so-1-cannot-open-shared-object-file-no-such-file-or-directory-with-cross-compiled-elf/post/5108791/#5108791)에 나와있듯이 NVML은 Tegra 플랫폼을 지원하지 않고, stub 라이브러리만 제공합니다. 테스트에 사용할 `cntk`라는 바이너리는 NVML을 필요로하기 때문에 stub 라이브러리를 사용해서 동작하도록 설정하겠습니다.

```
$ export LD_LIBRARY_PATH=/usr/local/cuda/targets/aarch64-linux/lib/stubs:$LD_LIBRARY_PATH
...
CPU로 돌리는 예제와 비교해 보면 훨씬 빠르게 학습이 진행됨을 알 수 있습니다.
